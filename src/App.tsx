...
export default function App() {
  ...
  // Store original status in a ref so onDragEnd can compare after optimistic updates
  ...
  function handleDragStart({ active }: DragStartEvent) {
    ...
    originalStatusRef.current = task?.status ?? null  // snapshot BEFORE any updates
  }
  ...
  function handleDragOver({ active, over }: DragOverEvent) {
    ...
    setTasks(prev => {
      ...
      if (overColumn) {
        if (activeTask.status === overColumn.id) return prev
        return prev.map(t => t.id === activeId ? { ...t, status: overColumn.id } : t)
      }
      ...
      if (activeTask.status !== overTask.status) {
        // Move to new column, place before the overTask
  ...
  async function handleDragEnd({ active }: DragEndEvent) {
    ...
    // Persist if column changed
    if (current.status !== original) {
      await moveTask(activeId, current.status, userId, original)
      await reload()  // re-fetch to ensure consistency
    }
  }
  ...
  function handleDragCancel() {
    ...
    if (activeTask && original) {
      setTasks(prev => prev.map(t => t.id === activeTask.id ? { ...t, status: original } : t))
    }
    setActiveTask(null)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1 }}>
      ...
      {/* Toolbar */}
      <div style={{ padding: '10px 24px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
        ...
        <button className="btn btn-ghost" style={{ fontSize: 13, padding: '7px 14px' }} onClick={() => setShowTeam(true)}>
          <Users size={14} /> Team
          {members.length > 0 && <span style={{ background: 'var(--accent)', color: 'white', fontSize: 11, fontWeight: 700, padding: '1px 6px', borderRadius: 99 }}>{members.length}</span>}
        </button>
        <button className="btn btn-ghost" style={{ fontSize: 13, padding: '7px 14px' }} onClick={() => setShowLabels(true)}>
          <Tag size={14} /> Labels
        </button>
        {members.length > 0 && (
            ...
            {members.slice(0, 6).map((m, i) => {
              ...
              return (
                <div key={m.id} title={m.name}
                  ...
                  style={{ width: 26, height: 26, borderRadius: '50%', background: m.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'white', border: filterAssignee === m.id ? '2px solid white' : '2px solid var(--surface)', marginLeft: i === 0 ? 0 : -8, cursor: 'pointer', zIndex: members.length - i, position: 'relative', boxShadow: `0 1px 4px ${m.color}55` }}>
                  {initials}
                </div>
              )
            })}
      ...
      >
        <div className="board-container">
          {COLUMNS.map((col, i) => (
            <div key={col.id} style={{ animation: `fadeIn 0.3s ease ${i * 0.07}s both` }}>
              <BoardColumn
                column={col}
        ...
        <DragOverlay>
          {activeTask && (
            <div style={{ transform: 'rotate(2deg)', boxShadow: '0 20px 60px rgba(0,0,0,0.6)', borderRadius: 12, pointerEvents: 'none' }}>
